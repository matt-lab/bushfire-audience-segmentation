...
population (as per the national 2016 census). Furthermore, regression
models (reported in the Supplementary Materials) do not find differences
between the mean age of participants across studies or the (log odds)
ratio of female to male participants across studies. We discarded the
data of extremely fast responders, who were categorised using a
preregistered threshold (see Supplementary Materials).

\begin{table}

\caption{\label{tbl-waves}Sample characteristics and materials of
study.}
...
\hspace{1em}Number of women in sample (\%) & 219 (50.34\%) & 198 (47.94\%) & 103 (48.36\%)\\
\addlinespace[0.3em]
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{qsort.pdf}
...
\caption{\label{tbl-scales}Summary of auxiliary psychological measures.}
...
time (from 27.13\% of Study 1 sample to 37.09\% of Study 3 sample). In
comparison, the proportion of Sceptics was relatively stable across
studies (from 8.28\% of Study 1 sample to 8.45\% of Study 3 sample). To
investigate whether the relative proportion of segments differed across
studies, we created a multinomial logistic regression model to predict
segment membership as a function of study (coefficients reported in
...
(\(\chi^{2}\) (4) = 8.85, \(p\) = 0.065, \(\omega =\) 0.09). Thus,
segment membership did not reliably differ across study samples.

\begin{figure}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{analysis_files/figure-pdf/fig-segment-change-1.pdf}}
...
greater mean endorsement of natural cycles causes for climate change
(e.g., volcanic eruptions, fluctuations in the sun) than participants of
Study 1 (Cohen's \emph{d} = 0.25). No other climate change cognition and
...
\caption{\label{tbl-mean_diff}Difference in means of climate change
cognition and affect characteristics between Study 1 and Study 3.}
...
membership, by replicating the regression analysis of
\citet{andreotta_2022}. This analysis is complicated by
...
\caption{\label{tbl-ridge-regression-segments-within-studies}Effect of
psychological characteristics on segment membership, as estimated by a
multinomial logistic ridge regression for Study 1 and Study 3.}
...
Appraisal}, was characterised by two items (items 2 and 4) which
...
Appraisal had an internal consistency of Cronbach's \(\alpha\) = 0.86
(mean inter-item correlation = 0.60) and Cronbach's \(\alpha\) = 0.42
(mean inter-item correlation = 0.29), respectively. Given the low
internal consistency of Fire Appraisal, we separated the factor into its
items---`Climate change made bushfires less likely' (item 2) and
`Bushfires were severe' (item 4)---for our following analysis.

To test segment differences in bushfire perceptions, we built linear
regression models predicting Climate Processes, the two Fire Appraisal
items, and Arson Causes as a function of segment membership
(coefficients reported in Supplementary Materials). All linear
regression models accounted for a significant amount of bushfire
perception variance compared to intercept-only models, indicating that
segment membership was a significant predictor of Climate Processes
(\(F\) (2, 210) = 47.44, \(p\) \textless{} .001, \(R^2\) = 0.31,
\(R^2_{adjusted}\) = 0.30), the two Fire Appraisal items (`Climate
change made bushfires less likely': \(F\) (2, 210) = 18.30, \(p\)
\textless{} .001, \(R^2\) = 0.15, \(R^2_{adjusted}\) = 0.14; `Bushfires
were severe': \(F\) (2, 210) = 19.71, \(p\) \textless{} .001, \(R^2\) =
0.16, \(R^2_{adjusted}\) = 0.15), and Arson Causes (\(F\) (2, 210) =
...
0.14, \emph{95\% CI} = {[}0.26, 0.80{]}, \emph{z} = 3.87, \(p\)
\textless{} .001, \(p_{adjusted}\) \textless{} .001), who in turn, had a
higher mean endorsement than Sceptics (difference = 1.76, \emph{SE} =
0.25, \emph{95\% CI} = {[}1.28, 2.24{]}, \emph{z} = 7.14, \(p\)
\textless{} .001, \(p_{adjusted}\) \textless{} .001). For the first Fire
Appraisal item of `Climate change made bushfires less likely',
Fencesitters had a higher mean endorsement than Acceptors (difference =
1.04, \emph{SE} = 0.17, \emph{95\% CI} = {[}0.70, 1.38{]}, \emph{z} =
6.05, \(p\) \textless{} .001, \(p_{adjusted}\) \textless{} .001) and
Sceptics (difference = 0.64, \emph{SE} = 0.31, \emph{95\% CI} = {[}0.04,
1.25{]}, \emph{z} = 2.08, \(p\) = .037, \(p_{adjusted}\) = .074).
However, Acceptors did not reliably differ from Sceptics in their mean
endorsement of the item (difference = -0.40, \emph{SE} = 0.30,
\emph{95\% CI} = {[}-0.99, 0.19{]}, \emph{z} = -1.33, \(p\) = .182,
\(p_{adjusted}\) = .182). For the second Fire Appraisal item of
`Bushfires were severe', Acceptors had a greater mean endorsement than
Sceptics (difference = 0.56, \emph{SE} = 0.19, \emph{95\% CI} = {[}0.20,
0.93{]}, \emph{z} = 3.04, \(p\) = .002, \(p_{adjusted}\) = .005) and
Fencesitters (difference = 0.65, \emph{SE} = 0.11, \emph{95\% CI} =
{[}0.44, 0.85{]}, \emph{z} = 6.05, \(p\) \textless{} .001,
\(p_{adjusted}\) \textless{} .001). However, Fencesitters did not
reliably differ from Sceptics in their mean endorsement of the item
(difference = -0.08, \emph{SE} = 0.19, \emph{95\% CI} = {[}-0.46,
0.29{]}, \emph{z} = -0.43, \(p\) = .664, \(p_{adjusted}\) = .664). The
pattern of Climate Processes endorsement was reversed for Arson Causes,
with Sceptics having a higher mean endorsement than Fencesitters
(difference = 0.74, \emph{SE} = 0.30, \emph{95\% CI} = {[}0.15, 1.32{]},
\emph{z} = 2.47, \(p\) = .014, \(p_{adjusted}\) = .014), who in turn,
had a higher mean endorsement than Acceptors (difference = 0.55,
\emph{SE} = 0.17, \emph{95\% CI} = {[}0.23, 0.88{]}, \emph{z} = 3.32,
\(p\) \textless{} .001, \(p_{adjusted}\) = .002).

\begin{table}

\caption{\label{tbl-fps-loadings}Items of the Fire Perception Scale,
their loadings onto each factor, their mean score, and their standard
deviation.}
...
Item & \parbox{5em}{\centering Climate Processes} & \parbox{5em}{\centering Fire Appraisal} & \parbox{5em}{\centering Arson Causes} & \textit{M} & \textit{SD}\\
\midrule
1. Climate change made the 2019-20 Australian bushfires more severe & \textbf{0.78} & 0.34 & -0.22 & 3.62 & 1.40\\
2. Climate change made the 2019-20 Australian bushfires less likely to occur & 0.27 & \textbf{-0.70} & \textbf{0.42} & 2.19 & 1.27\\
3. The 2019-20 Australian bushfires have accelerated climate change & \textbf{0.84} & 0.05 & -0.14 & 3.16 & 1.30\\
4. The 2019-20 Australian bushfires are severe & 0.17 & \textbf{0.86} & 0.23 & 4.50 & 0.79\\
5. If the government increased taxes on all fossil fuels (e.g., gasoline, oil, coal, kerosene), Australia would be less likely to experience extreme bushfires & \textbf{0.84} & -0.19 & 0.13 & 2.55 & 1.32\\
6. If we changed our lifestyles to reduce our consumption, Australia would be less likely to experience bushfires & \textbf{0.86} & -0.06 & 0.08 & 3.05 & 1.39\\
7. Over one hundred arsonists have contributed to the 2019-20 Australian bushfires & -0.10 & 0.04 & \textbf{0.94} & 3.47 & 1.20\\
\bottomrule
\multicolumn{6}{l}{\rule{0pt}{1em}\textit{Note: }}\\
...
\pandocbounded{\includegraphics[keepaspectratio]{analysis_files/figure-pdf/fig-fps-segment-1.pdf}}
...
(\emph{r} = -0.21, 95\% \emph{CI} = {[}-0.33, -0.08{]}, \emph{p} =
.002), which suggests that the two causal accounts for the Black Summer
bushfires were somewhat incompatible.

To test a motivated-reasoning account of perceptions of the bushfires,
we examined the association between political liberalism-conservatism
(as indexed by the single-item measure of political ideology)---a proxy
for pre-existing climate change beliefs---and endorsement of the
climate-change and mass-arson causal claims. Political liberalism was
associated with stronger endorsement of the climate-change causal claim
(\emph{r} = -0.34, 95\% \emph{CI} = {[}-0.45, -0.21{]}, \emph{p}
\textless{} .001), whereas political conservatism was associated with
stronger endorsement of the mass-arson causal claim (\emph{r} = 0.28,
95\% \emph{CI} = {[}0.15, 0.40{]}, \emph{p} \textless{} .001). However,
both correlations were relatively weak, indicating that acceptance of
anthropogenic climate change was not tied solely to pre-existing
political beliefs, and that the mass-arson causal claim meshed with a
broad range of respondents---including some who accept anthropogenic
climate change---providing only limited support for a
motivated-reasoning explanation (see Supplementary Materials for
correlations between the other auxiliary psychological characteristic
measures and the Fire Perception Scale causal items).
...
\textless{} .001, Cohen's \(\omega\) = 0.43). Specifically, we found
that the odds of Acceptors (\(n\) = 104, 89.66\% of Acceptors, odds =
8.67) indicating a preference for more governmental climate change
mitigation policies were approximately eight times greater (odds ratio =
8.03, 95\% \emph{CI} = {[}3.92, 17.49{]}, \(p\) \textless{} .001) than
Fencesitters (\(n\) = 41, 51.90\% of Fencesitters, odds = 1.08).
...
(e.g., one participant wrote ``the recent bushfire is a wakupe call. how
much more \emph{worse} do we want to experience?'', fear words
italicised) and for no changes or less action (e.g., one participant
wrote ``\ldots100 arsonists were charged as a starter and the it was the
fuel left on the ground for decades that made the fires so much
...
of Fencesitters, odds = 0.22). Sceptics did not reliably differ in their
use of fear words (\(n\) = 6, 33.33\% of Sceptics, odds = 0.50) from
Acceptors or Fencesitters.
...
\pandocbounded{\includegraphics[keepaspectratio]{analysis_files/figure-pdf/fig-policy-support-1.pdf}}

}

\caption{\label{fig-policy-support}Policy direction preferences as a
proportion of each segment. Error bars indicate one standard error of
the proportion.}
