...
(mean inter-item correlation = 0.29), respectively.
...
(difference = 0.48, \emph{SE} = 0.19, \emph{95\% CI} = {[}0.11, 0.85{]},
\emph{z} = 2.54, \(p\) = .011, \(p_{adjusted}\) = .022) and Fencesitters
(difference = 0.84, \emph{SE} = 0.11, \emph{95\% CI} = {[}0.63, 1.06{]},
\emph{z} = 7.75, \(p\) \textless{} .001, \(p_{adjusted}\) \textless{}
.001). However, Fencesitters did not reliably differ from Sceptics in
their mean endorsement of Fire Realities (difference = -0.36, \emph{SE}
= 0.20, \emph{95\% CI} = {[}-0.75, 0.02{]}, \emph{z} = -1.86, \(p\) =
.063, \(p_{adjusted}\) = .063). The pattern of Climate Processes
endorsement was reversed for Arson Causes, with Sceptics having a higher
mean endorsement than Fencesitters (difference = 0.74, \emph{SE} = 0.30,
\emph{95\% CI} = {[}0.15, 1.32{]}, \emph{z} = 2.47, \(p\) = .014,
\(p_{adjusted}\) = .014), who in turn, had a higher mean endorsement
than Acceptors (difference = 0.55, \emph{SE} = 0.17, \emph{95\% CI} =
{[}0.23, 0.88{]}, \emph{z} = 3.32, \(p\) \textless{} .001,
\(p_{adjusted}\) = .002).

\hypertarget{tbl-fps-loadings}{}
\begin{table}
\caption{\label{tbl-fps-loadings}Items of the Fire Perception Scale, their loadings onto each factor,
their mean score, and their standard deviation. }\tabularnewline
