...
get_chisq_text <- function(chisq, df, p_value, p_value_adjusted = NULL, w = NULL) {
    ...
    str_c(
        ...
        ifelse(
            is.null(w),
            "",
            str_c(", Cohen's $\\omega$ = ", specify_decimal(w))
        )
    )
}
...
Furthermore, regression models (reported in the Supplementary Materials) do not find differences between the mean age of participants across studies or the (log odds) ratio of female to male participants across studies.
...
tab_waves <- data |>
    ...
    mutate(across(
        ...
        ~ str_c(
            ...
            '-',
            str_to_title(month(.x, label = TRUE)),
            '-',
            year(.x)))) |>
    arrange(study) |>
...
In comparison, the proportion of Sceptics was relatively stable across studies `r pull(filter(descriptives_segment_change, segment == 'sceptic'), text)`.
To investigate whether the relative proportion of segments differed across studies, we created a multinomial logistic regression model to predict segment membership as a function of study (coefficients reported in Supplementary Material), using the *multinom* function from the *nnet* package [@venables_2002].
A log likelihood ratio test did not indicate an improvement in model fit when study was included as a predictor, compared to a model with only an intercept term ($\chi^{2}$ (`r lr_test_segment_change$df`) = `r specify_decimal(lr_test_segment_change$lr_stat)`, $p$ = `r specify_decimal(lr_test_segment_change$pr_chi, 3)`, $\omega =$ `r specify_decimal(lr_test_segment_change$w)`).
...
data |>
    ...
    ggplot(aes(x = segment_name, y = percentage, fill = study_name_full, colour = study_name_full)) +
    geom_bar_pattern(
        aes(
            pattern = study_name_full
        ), 
        colour = 'black',
        pattern_fill = colour_hatch_pattern,
        pattern_colour = colour_hatch_pattern,
        pattern_density = density_hatch_pattern,
        position = "dodge",
        stat = "identity",
    ...
    scale_x_discrete(labels = c("Acceptors", "Fence-sitters", "Sceptics")) +
...
We found evidence of a statistically significant mean difference in climate change cognition and affect (@tbl-mean_diff). Specifically, participants in Study 3 had a greater mean endorsement of natural cycles causes for climate change (e.g., volcanic eruptions, fluctuations in the sun) than participants of Study 1 (Cohen's *d* = `r specify_decimal(filter(means_diff_results, scale == 'mms_cau_n')$cohens_d, 2)`).
...
We explored the psychological characteristics associated with segment membership, by replicating the regression analysis of @andreotta_2022.
...
fps_avg_item_correlations <-
    ...
    summarise(
        r_avg = mean(estimate),
        r_avg_rounded = specify_decimal(r_avg)
    )

# Given low internal reliability of Fire Realities, we split Fire Realities up
...
fps_models_marginal_means <-
    ...
    rowwise() |>
    mutate(
        conf = str_flatten_comma(specify_decimal(sort(c(conf_low,conf_high)))),
        conf_rev = str_flatten_comma(specify_decimal(sort(-c(conf_low,conf_high))))
    ) |>
    ungroup() |>
    mutate(
        text = str_c(
...
The second factor, labelled *Fire Appraisal*, was characterised by two items (items 2 and 4) which participants generally responded with certainty and accounted for `r fps_pca_var[2]`% of scale variance.
...
The multi-item factors of Climate Processes and Fire Appraisal had an internal consistency of Cronbach's $\alpha$ = `r fps_alphas_rounded[[1]][1]` (mean inter-item correlation = `r fps_avg_item_correlations$r_avg_rounded[1]`) and Cronbach's $\alpha$ = `r fps_alphas_rounded[[2]][1]` (mean inter-item correlation = `r fps_avg_item_correlations$r_avg_rounded[2]`), respectively.
Given the low internal consistency of Fire Appraisal, we separated the factor into its items---'Climate change made bushfires less likely' (item 2) and 'Bushfires were severe' (item 4)---for our following analysis.

To test segment differences in bushfire perceptions, we built linear regression models predicting Climate Processes, the two Fire Appraisal items, and Arson Causes as a function of segment membership (coefficients reported in Supplementary Materials).
All linear regression models accounted for a significant amount of bushfire perception variance compared to intercept-only models, indicating that segment membership was a significant predictor of Climate Processes (`r fps_models_fits[1]`), the two Fire Appraisal items ('Climate change made bushfires less likely': `r fps_models_fits[2]`; 'Bushfires were severe': `r fps_models_fits[3]`), and Arson Causes (`r fps_models_fits[4]`).
...
As seen in @fig-fps-segment, Acceptors had a higher mean endorsement of Climate Processes than Fencesitters (`r filter(fps_models_marginal_means, str_detect(response, "Climate Processes") & contrast == "Acceptors - Fencesitters")$text`), who in turn, had a higher mean endorsement than Sceptics (`r filter(fps_models_marginal_means, str_detect(response, "Climate Processes") & contrast == "Sceptics - Fencesitters")$text_rev`).
For the first Fire Appraisal item of 'Climate change made bushfires less likely', Fencesitters had a higher mean endorsement than Acceptors (`r filter(fps_models_marginal_means, str_detect(response, "Climate change made bushfires less likely") & contrast == "Acceptors - Fencesitters")$text_rev`) and Sceptics (`r filter(fps_models_marginal_means, str_detect(response, "Climate change made bushfires less likely") & contrast == "Sceptics - Fencesitters")$text_rev`). However, Acceptors did not reliably differ from Sceptics in their mean endorsement of the item (`r filter(fps_models_marginal_means, str_detect(response, "Climate change made bushfires less likely") & contrast == "Acceptors - Sceptics")$text`).
For the second Fire Appraisal item of 'Bushfires were severe', Acceptors had a greater mean endorsement than Sceptics (`r filter(fps_models_marginal_means, str_detect(response, "Bushfires were severe") & contrast == "Acceptors - Sceptics")$text`) and Fencesitters (`r filter(fps_models_marginal_means, str_detect(response, "Bushfires were severe") & contrast == "Acceptors - Fencesitters")$text`).
However, Fencesitters did not reliably differ from Sceptics in their mean endorsement of the item (`r filter(fps_models_marginal_means, str_detect(response, "Bushfires were severe") & contrast == "Sceptics - Fencesitters")$text_rev`).
The pattern of Climate Processes endorsement was reversed for Arson Causes, with Sceptics having a higher mean endorsement than Fencesitters (`r filter(fps_models_marginal_means, str_detect(response, "Arson Causes") & contrast == "Sceptics - Fencesitters")$text`), who in turn, had a higher mean endorsement than Acceptors (`r filter(fps_models_marginal_means, str_detect(response, "Arson Causes") & contrast == "Acceptors - Fencesitters")$text_rev`).

```{r}
#| label: tbl-fps-loadings
#| tbl-cap: "Items of the Fire Perception Scale, their loadings onto each factor, their mean score, and their standard deviation."
#| echo: false
...
tab_fps_loadings_names <- c(
    ...
    sprintf(r"{\parbox{5em}{\centering %s}}", "Fire Appraisal"),
...
# For journal style
names(segment_colours) <- sort(unique(fct_recode(
          data$segment_name,
          `Fence-sitters` = "Fencesitters"
        )))
str_to_sentence_with_colon = function(str) {
  str |>
  str_split(":") |>
  map(str_to_sentence) |>
...
data_fps_scales_revised |>
    ...
    mutate(factor_name = fct_relabel(factor_name, str_to_sentence_with_colon)) |>
    mutate(factor_name = fct_reorder(factor_name, factor)) |>
    arrange(factor_name) |>
    ...
    scale_fill_manual(
        "Segment",
        values = segment_colours #segment_colours
    ) +
    theme(
...
text_cor_fps_1_fps_7 <-
    ...
    mutate(
        text = str_c(
            ...
            glue(", 95% *CI* = [{specify_decimal(conf.low)}, {specify_decimal(conf.high)}]"),
            glue(", *p* ", specify_p_value_text(p.value, with_star = F, with_equals = T, 3))
        )
    ) |>
    pull(text)
...
data_fps_causal_items <-
  ...
  select(id, item, value_scored) |>
  rename(fps_item = item) |>
  rename(fps_score = value_scored) |>
  mutate(study = 3)

text_cor_causal_items_pi <-
  data |>
    ...
    mutate(
        text = str_c(
            glue("*r* = ", specify_decimal(estimate)),
            glue(", 95% *CI* = [{specify_decimal(conf.low)}, {specify_decimal(conf.high)}]"),
            glue(", *p* ", specify_p_value_text(p.value, with_star = F, with_equals = F, 3))
        )
    ) |>
    ungroup()
...
To test a motivated-reasoning account of perceptions of the bushfires, we examined the association between political liberalism-conservatism (as indexed by the single-item measure of political ideology)---a proxy for pre-existing climate change beliefs---and endorsement of the climate-change and mass-arson causal claims. Political liberalism was associated with stronger endorsement of the climate-change causal claim (`r pull(filter(text_cor_causal_items_pi, fps_item == 'fps_1'), text)`), whereas political conservatism was associated with stronger endorsement of the mass-arson causal claim (`r pull(filter(text_cor_causal_items_pi, fps_item == 'fps_7'), text)`). However, both correlations were relatively weak, indicating that acceptance of anthropogenic climate change was not tied solely to pre-existing political beliefs, and that the mass-arson causal claim meshed with a broad range of respondents---including some who accept anthropogenic climate change---providing only limited support for a motivated-reasoning explanation (see Supplementary Materials for correlations between the other auxiliary psychological characteristic measures and the Fire Perception Scale causal items).
    ...
    mutate(
        text =
            get_chisq_text(
                ...
                df,
                p_value,
                w = sqrt(statistic / nrow(data_segment_mitigation))
            )
    )
...
The most common emotion evoked by participants was fear (`r sentiment_fp_mitigation_descriptives$n_text[sentiment_fp_mitigation_descriptives$sentiment == "fear"]`), found in both justification for more action (e.g., one participant wrote "the recent bushfire is a wakupe call. how much more *worse* do we want to experience?", fear words italicised) and for no changes or less action (e.g., one participant wrote "...100 arsonists were charged as a starter and the it was the fuel left on the ground for decades that made the fires so much *worse* and caused the *disaster*", fear words italicised).
...
Sceptics did not reliably differ in their use of fear words (`r filter(sentiment_fp_mitigation_segment_descriptives, segment == 'sceptic')$text`) from Acceptors or Fencesitters.


```{r}
#| label: fig-policy-support
#| fig-cap: "Policy direction preferences as a proportion of each segment. Error bars indicate one standard error of the proportion."
#| fig-width: 6.5
...
data |>
    ...
    labs(
        x = "Policy direction preferences",
