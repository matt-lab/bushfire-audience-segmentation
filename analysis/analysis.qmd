...
A log likelihood ratio test did not indicate an improvement in model fit when study was included as a predictor, compared to a model with only an intercept term ($\chi^{2}$ (`r lr_test_segment_change$df`) = `r specify_decimal(lr_test_segment_change$lr_stat)`, $p$ = `r specify_decimal(lr_test_segment_change$pr_chi)`, $\omega =$ `r specify_decimal(lr_test_segment_change$w)`).
