...
The multi-item factors of Climate Processes and Fire Realities had an internal consistency of Cronbach's $\alpha$ = `r fps_alphas_rounded[[1]][1]` (mean inter-item correlation = `r fps_avg_item_correlations$r_avg_rounded[1]`) and Cronbach's $\alpha$ = `r fps_alphas_rounded[[2]][1]` (mean inter-item correlation = `r fps_avg_item_correlations$r_avg_rounded[2]`), respectively.
Given the low internal consistency of Fire Realities, we separated the factor into its items---'Climate change made bushfires less likely' (item 2) and 'Bushfires were severe' (item 4)---for our following analysis.

To test segment differences in bushfire perceptions, we built linear regression models predicting Climate Processes, the two Fire Realities items, and Arson Causes as a function of segment membership (coefficients reported in Supplementary Materials).
All linear regression models accounted for a significant amount of bushfire perception variance compared to intercept-only models, indicating that segment membership was a significant predictor of Climate Processes (`r fps_models_fits[1]`), the two Fire Realities items ('Climate change made bushfires less likely': `r fps_models_fits[2]`; 'Bushfires were severe': `r fps_models_fits[3]`), and Arson Causes (`r fps_models_fits[4]`).
