...
colour_grid <- "#A0A0A0"
colour_axis <- "#000000"
colour_hatch_pattern <- "gray35"
density_hatch_pattern <- 0.15
...
study_colours <- rep('white', length(unique(data$study_name_full))) #c("#6699CC", "#661100", "#882255")
names(study_colours) <- sort(unique(data$study_name_full))
segment_colours <- rep('white', length(unique(data$segment_name))) #c("#117733", "#DDCC77", "#88CCEE")
...
data_fps_scales <- data |>
    ...
    mutate(factor_name = c("(A) Climate Processes", "(B) Fire Appraisal", "(C) Arson Causes")[factor]) |>
...
tab_gender_diff_names <- c(
    "Predictors",
    "Categories",
    "Estimate ($p$ value)",
    "95\\% CI"
)
tab_gender_diff_footnotes <-
    c(
        "\\\\parbox{31em}{Each study was entered as a categorical predictor, with Study 2 as the reference category. Male was entered as the reference category of gender. Model estimates of coefficients were exponentiated to odds ratios.}"
    )
...
data_gender <-
    ...
    mutate(gender = fct_relevel(gender, "1")) |>
    mutate(study = fct_relevel(study_name, ref_study))
model_gender_diff <- multinom(gender ~ study, data_gender, trace = FALSE)
lr_test_gender_diff <-
    anova(
        multinom(gender ~ 1, data_gender, trace = FALSE),
        multinom(gender ~ study, data_gender, trace = FALSE)
    ) |>
    janitor::clean_names() |>
    filter(
        model == 'study'
...
data |>
    ...
    kbl(
        ...
        escape = FALSE,
        booktabs = TRUE,
        col.names = tab_gender_diff_names
     ) |>
#    kable_styling(full_width = TRUE) |>
    add_header_above(
        c(
            " " = 2,
            r"{\\parbox{18em}{\\centering Ratio of female odds and male odds}}" = 2
        ),
        escape = FALSE
    ) |>
    ...
    footnote(
        general = tab_gender_diff_footnotes,
        escape = FALSE
    )
...
data_age <-
    data |>
    mutate(study = fct_relevel(study_name, ref_study))
model_age_diff <- lm(age ~ study, data_age)
...
data |>
    ...
    footnote(
        general =
            ...
            "\\\\parbox{28em}{Each study was entered as a categorical predictor, with Study 2 as the reference category.}"
        ),
        escape = FALSE
    )
#
...
tab_segment_change_footnotes <-
    c(
        ...
        "\\\\parbox{36em}{Each study was entered as a categorical predictor, with Study 2 as the reference category. Fence-sitter was entered as the reference category of segment. Model estimates of coefficients were exponentiated to odds ratios.}"
    )
...
data |>
    ...
    relocate(ends_with("_Acceptors"), .after = "categories") |>
    mutate(
        variables = case_when(
            variables == lag(variables, 1)
                ~ "",
...
#    kable_styling(full_width = TRUE) |>
    add_header_above(
        c(
            " " = 2,
            r"{\\parbox{10em}{\\centering Ratio of Acceptors odds and Fence-sitters odds}}" = 2,
            r"{\\parbox{10em}{\\centering Ratio of Sceptics odds and Fence-sitters odds}}" = 2
        ),
        escape = FALSE
    ) |>
    column_spec(1:2, width = "6em") |>
...
means_diff_results <- data |>
    ...
    mutate(
        test = map2(study_3, study_1, t.test),
...
tab_mean_diff_results <- means_diff_results |>
    ...
    mutate(p_adjusted = specify_p_value(p_adjusted, 3)) |>
...
tab_mean_diff_results <- tab_mean_diff_results |>
    ...
    add_header_above(
        c(
            " " = 1,
            r"{\\textit{$M_{Study\\ 3} - M_{Study\\ 1}$}}" = 2,
            " " = 3),
        escape = FALSE) |>
    footnote(
        general =
...
# Given low internal reliability of Fire Appraisal, we split Fire Appraisal up
# into its original items
# We also retain the original values for item 2, rather than reverse code
data_fps_scales_revised <-
  data_fps_scales |>
  mutate(value_scored = value) |>
  mutate(
    factor =
      case_when(
        factor == 1 ~ 1,
        factor == 3 ~ 4,
        factor == 2 & item == 'fps_2' ~ 2,
        factor == 2 & item == 'fps_4' ~ 3
      )
  ) |>
  mutate(
    factor_name =
      case_when(
        item == 'fps_2' ~ str_c(factor_name, ": Climate change made bushfires less likely"),
        item == 'fps_4' ~ str_c(factor_name, ": Bushfires were severe"),
        TRUE ~ factor_name
      )
  ) |>
  mutate(factor_name = fct_reorder(factor_name, factor))
...
data |>
    ...
    select(variables, categories, estimate, conf) |>
    mutate(
      categories = ifelse(categories == "Fencesitters", "Fence-sitters", categories)
    ) |>
    mutate(
        variables = case_when(
            variables == lag(variables, 1)
    ...
    pack_rows(
        unique(coefs_fps_models$response)[4],
        13,
        13 + 3
    ) |>
    footnote(
        general =
            ...
            "\\\\parbox{30em}{Each segment was entered as a categorical predictor, with Fence-sitter as the reference category.}"
        ),
        escape = FALSE
    )
#
...
data_fps_correlations <- 
    ...
    focus(
      `Climate Processes`,
      `Arson Causes`,
      `Fire Appraisal: Climate change made bushfires less likely`,
      `Fire Appraisal: Bushfires were severe`
    ) |>
    ...
    mutate(
        fps = factor(
            ...
            labels = levels(data_fps_scales_revised$factor_name)
        )
    ) |>
    left_join(
        scales_info,
    ...
    mutate(
        p_value_markers =
            get_p_value_markers(
                case_when(
                    abs(r) > cor_critical(.001, n_distinct(data_fps_scales$id)) ~ .001 - 1e-3,
                    abs(r) > cor_critical(.01, n_distinct(data_fps_scales$id)) ~ .01 - 1e-3,
                    abs(r) > cor_critical(.05, n_distinct(data_fps_scales$id)) ~ .05 - 1e-3,
                    TRUE ~ 1
                )
            )
    ) |>
    mutate(
        r_colour =
            case_when(
                r > cor_critical(.05, n_distinct(data_fps_scales$id)) ~ tail(carto_pal(201, magnitude_palette), 1),
                r < -cor_critical(.05, n_distinct(data_fps_scales$id)) ~ head(carto_pal(201, magnitude_palette), 1),
                TRUE ~ "#808080"
...
data_fps_correlations |>
    ...
    column_spec(c(2, 3, 4, 5), width = "4.5em") |>
    kable_styling(
        latex_options = c("repeat_header")
    ) |>
    add_header_above(
...
# Some exploration of correlations
data_fps_causal_items <-
  data_fps_scales |>
  filter(item %in% c('fps_1', 'fps_7'))

data_fps_correlations_causal_items <- 
    data |>
    select(any_of(scales_info$abbrev), study, id) |>
    pivot_longer(-c(study, id), names_to = 'scale', values_to = 'score') |>
    left_join(scales_info, by = c('scale' = 'abbrev')) |>
    ...
    add_case(
        study = 3,
        id = data_fps_causal_items$id,
        scale = data_fps_causal_items$item,
        score = data_fps_causal_items$value_scored
    ) |>
    mutate(
      measure = case_when(
        scale == "fps_1" ~ "1. Climate change causes",
        scale == "fps_7" ~ "7. Arson causes",
        TRUE ~ measure
      )
    ) |>
    filter(study == 3) |>
    ...
    focus(`1. Climate change causes`, `7. Arson causes`) |>
    pivot_longer(-term, names_to = "fps", values_to = "r") |>
    # Prepare table data
    mutate(
        fps = factor(
            fps,
            levels = c("1. Climate change causes", "7. Arson causes")
        )
    ) |>
    left_join(
        scales_info,
...
data_fps_correlations_causal_items |>
    ...
    pack_groups(data_fps_correlations_causal_items, category) |>
    ...
    add_header_above(
        c(
            " " = 1,
            "Fire Perception casual items" = 2
        )
    ) |>
    footnote(
        general =
...
tab_segment_mitigation_footnotes <-
    c(
        ...
        "\\\\parbox{34em}{Each segment was entered as a categorical predictor, with Fence-sitter as the reference category. Sceptics were excluded from the model, as none indicated a preference for more action. Model estimates of coefficients were exponentiated to odds ratios.}"
    )
...
data |>
    ...
    mutate(categories = ifelse(categories == "Fencesitters", "Fence-sitters", categories)) |>
    mutate(
        variables = case_when(
            variables == lag(variables, 1)
                ~ "",
...
model_sentiment |>
    ...
    mutate(
        ...
        p_value = specify_p_value_text(p_value, with_stars = T, with_equals = F, 3),
    ...
    mutate(contrast_text = str_replace(contrast_text, "Fencesitters", "Fence-sitters")) |>
    kbl(
        align = "l",
        escape = FALSE,
        booktabs = TRUE,
    ...
    footnote(
        general =
            c(
                ...
                "\\\\parbox{32em}{\\\\textit{p} values for contrasts were adjusted using the \\\\citet{holm1979} method. Estimates were exponentiated.}"
            ),
        escape = FALSE
    )
