...
To investigate specific study effects on segment membership, we followed up with pairwise comparisons between studies, using the *marginaleffects* package [@R-marginaleffects].
We controlled for multiple comparisons by adjusting $p$ values with the Holm method [@holm1979].
Pairwise comparisons are presented in @tbl-segment-change-contrasts.
...
tab_segment_change_footnotes <-
    c(
        ...
        "\\\\parbox{36em}{Each study was entered as a categorical predictor, with Study 2. Fencesitter was entered as the reference category of segment. Model estimates of coefficients were exponentiated to odds ratios.}"
    )
...
```{r}
#| label: tbl-segment-change-contrasts
#| tbl-cap: "Pairwise comparisons of study on segment membership, estimated using a multinomial logistic regression model."

#tab_segment_change_contrasts <-
  get_contrasts(
    model = model_segment_change,
    variables = list(study = 'pairwise'),
    newdata = 'balanced',
